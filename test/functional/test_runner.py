...
`test/functional/test_framework/test_framework.py:MulecoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/mulecoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    logging.debug("Temporary test directory at %s" % tmpdir)

    # Don't run tests on Windows by default
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/MulecoinProject/Mulecoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/MulecoinProject/Mulecoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    # Check that the build was configured with wallet, utils, and mulecoind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_mulecoind = config["components"].getboolean("ENABLE_MULECOIND")
    if not (enable_wallet and enable_cli and enable_mulecoind):
        print("No functional tests to run. Wallet, utils, and mulecoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if mulecoind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "mulecoind"]) is not None:
            print("%sWARNING!%s There is already a mulecoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "MULECOIND" not in os.environ:
        os.environ["MULECOIND"] = build_dir + '/src/mulecoind' + exeext
        os.environ["MULECOINCLI"] = build_dir + '/src/mulecoin-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `mulecoin-cli help` (`rpc_interface.txt`).
