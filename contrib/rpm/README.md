...
The RPM spec file provided here is for Mulecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://mulecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Mulecoin git repository](https://github.com/MulecoinProject/Mulecoin) but are not part of
...
Currently only `Source10` is used, for the example `mulecoin.conf` file.
...
in the mulecoin git repository. At present only an SVG version of the Mulecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the mulecoin
...
needed when 0.12.1 is released, a proper fix is already in the Mulecoin git
master branch.
...
idea to build Mulecoin Core against a static version of that library compiled
...
The `mulecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `mulecoin-server` package. At this point, most RPM based
...
The `mulecoin-server` package should have SELinux support. How to properly do
...
Mulecoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/mulecoin/). He has been
packaging Mulecoin for Fedora at least since 2012.
...
4. I renamed the `mulecoin` package that contains the Qt GUI to `mulecoin-core` as
that appears to be how the general population refers to it, in contrast to
`mulecoin-xt` or `mulecoin-classic`. I wanted to make sure the general population
