...
#include "mulecoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Mulecoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new MulecoinAddressEntryValidator(parent));
    widget->setCheckValidator(new MulecoinAddressCheckValidator(parent));
}
...
bool parseMulecoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no mulecoin: URI
    if(!uri.isValid() || uri.scheme() != QString("mulecoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MulecoinUnits::parse(MulecoinUnits::FOXD, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMulecoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert mulecoin:// to mulecoin:
    //
    //    Cannot handle this later, because mulecoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("mulecoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "mulecoin:");
    }
    QUrl uriInstance(uri);
    return parseMulecoinURI(uriInstance, out);
}

QString formatMulecoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("mulecoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MulecoinUnits::format(MulecoinUnits::FOXD, info.amount, false, MulecoinUnits::separatorNever));
        paramCount++;
    }
...
bool openMulecoinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(MULECOIN_CONF_FILENAME);
    ...
    /* Open mulecoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mulecoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mulecoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Mulecoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Mulecoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "mulecoin.desktop";
    return GetAutostartDir() / strprintf("mulecoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a mulecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Mulecoin\n";
        else
            optionFile << strprintf("Name=Mulecoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the mulecoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef mulecoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (mulecoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, mulecoinAppUrl);

    CFRelease(mulecoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add mulecoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, mulecoinAppUrl, nullptr, nullptr);
    }
