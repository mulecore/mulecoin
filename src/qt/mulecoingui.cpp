...
#include "mulecoingui.h"

#include "mulecoinunits.h"
...
const std::string MulecoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MulecoinGUI::DEFAULT_WALLET = "~Default";

MulecoinGUI::MulecoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void MulecoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Mulecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mulecoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to FOXD addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Mulecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Mulecoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a mulecoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Mulecoin command-line options").arg(tr(PACKAGE_NAME)));
...
void MulecoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void MulecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** FOXD START */
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* mulecoinLabelLayout = new QVBoxLayout(toolbarWidget);
        mulecoinLabelLayout->addWidget(label);
        mulecoinLabelLayout->addWidget(toolbar);
        mulecoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        mulecoinLabelLayout->addStretch(1);
...
void MulecoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MulecoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MulecoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MulecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MulecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void MulecoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void MulecoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MulecoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MulecoinGUI::optionsClicked()
{
...
void MulecoinGUI::showDebugWindow()
{
...
void MulecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MulecoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void MulecoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MulecoinGUI::openClicked()
{
...
void MulecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MulecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MulecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MulecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MulecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MulecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** FOXD START */
void MulecoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void MulecoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void MulecoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void MulecoinGUI::gotoRestrictedAssetsPage()
{
...
void MulecoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Mulecoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MulecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MulecoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MulecoinGUI::updateHeadersSyncProgressLabel()
{
...
void MulecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void MulecoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Mulecoin"); // default title
...
void MulecoinGUI::changeEvent(QEvent *e)
{
...
void MulecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MulecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MulecoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "FOXD")
        msg += tr("Amount: %1\n").arg(MulecoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(MulecoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void MulecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MulecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MulecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MulecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MulecoinGUI::setHDStatus(int hdEnabled)
{
...
void MulecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MulecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MulecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MulecoinGUI::detectShutdown()
{
...
void MulecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MulecoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MulecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MulecoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void MulecoinGUI::subscribeToCoreSignals()
{
...
void MulecoinGUI::unsubscribeFromCoreSignals()
{
...
void MulecoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void MulecoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<MulecoinUnits::Unit> units = MulecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MulecoinUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(MulecoinUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(MulecoinUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (MulecoinUnits::Unit u : MulecoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MulecoinUnits::name(u)), this);
...
{
    setText(MulecoinUnits::name(newUnits));
}
...
void MulecoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=FOXDBTC"));
