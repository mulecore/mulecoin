...
{
    // Call into TxToUniv() in mulecoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in mulecoin-common, so we query them here and push the
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    ...
                    if (spentInfo.addressType == 1) {
                        in.pushKV("address", CMulecoinAddress(CKeyID(spentInfo.addressHash)).ToString());
                    } else if (spentInfo.addressType == 2) {
                        in.pushKV("address", CMulecoinAddress(CScriptID(spentInfo.addressHash)).ToString());
                    }
                }
                newVin.push_back(in);
            }
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Outputs are addresses (paired with a FOXD amount, data or object specifying an asset operation) or data.\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction's inputs are not signed, and\n"
            "it is not stored in the wallet or transmitted to the network.\n"

            "\nPaying for Asset Operations:\n"
            "  Some operations require an amount of FOXD to be sent to a burn address:\n"
            "\n"
            ...
            "     {\n"
            "       \"address\":                          (string, required) The destination mulecoin address.\n"
            "                                               Each output must have a different address.\n"
            "         x.xxx                             (number or string, required) The FOXD amount\n"
            "           or\n"
            ...
            // TODO if we decide to remove the consensus check from issue 675 https://github.com/MulecoinProject/Mulecoin/issues/675
...
// TODO, if we decide to remove the consensus check https://github.com/MulecoinProject/Mulecoin/issues/675, remove or add the code (requires consensus change)
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) mulecoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
