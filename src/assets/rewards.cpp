...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for FOXD
    ...
    if (p_rewardSnapshot.strDistributionAsset != "FOXD") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is FOXD with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using FOXD differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "FOXD") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Mulecoin address (already validated during ownership snapshot creation)
