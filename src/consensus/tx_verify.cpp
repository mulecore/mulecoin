...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs, bool fFromMempool, bool fFromBlock)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        /** FOXD START */
        // Find and handle all new OP_FOXD_ASSET null data transactions
    ...
    if (tx.IsNewAsset()) {
        ...
        // above transaction types.  Also fail if it contains OP_FOXD_ASSET opcode but wasn't a valid script.
        for (auto out : tx.vout) {
            int nType;
            bool _isOwner;
            if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                ...
                if (out.scriptPubKey.Find(OP_FOXD_ASSET)) {
                    if (out.scriptPubKey[0] != OP_FOXD_ASSET) {
                        return state.DoS(100, false, REJECT_INVALID,
