...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        BOOST_CHECK(!IsAssetNameValid("FOXD", type));
        BOOST_CHECK(!IsAssetNameValid("MULECOIN", type));
        BOOST_CHECK(!IsAssetNameValid("MULECOIN", type));

        //- Versions of MULECOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("MULECOIN.COIN", type));
        BOOST_CHECK(IsAssetNameValid("MULECOIN_COIN", type));
        BOOST_CHECK(IsAssetNameValid("FOXDSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERFOXD", type));
        BOOST_CHECK(IsAssetNameValid("MULECOINSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEMULECOIN", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_MULECOINS", type));
        BOOST_CHECK(IsAssetNameValid("SEFOXDOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("MULECOIN", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("MULECOIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
