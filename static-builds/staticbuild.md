...
make dist
SOURCEDIST=`echo mulecoin-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find mulecoin-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/mulecoin
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/mulecoin-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer mulecoin-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer mulecoin-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
#transfer mulecoin-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/mulecoin-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer mulecoin-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
